...
export default function ChatLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-[calc(100vh-8rem)] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm md:h-[calc(100vh-3rem)]">
      <div
        className={cn(
          "w-full shrink-0 flex-col border-slate-200 md:flex md:w-80 md:border-r",
          isConversationOpen ? "hidden" : "flex",
        )}
      >
        <div className="flex items-center justify-between gap-3 border-b border-slate-100 p-5 pb-4">
          <h1 className="text-xl font-semibold text-slate-900">Chat</h1>
          <Link
            href="/friends"
            aria-label="Start a new chat"
            title="Start a new chat"
            className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-indigo-600 hover:bg-indigo-50"
          >
        ...
        <div className="border-b border-slate-100 px-5 py-3">
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2.5 pl-9 pr-4 text-sm text-slate-900 placeholder:text-slate-400 focus:border-indigo-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/30"
            />
          </div>
                ...
                return (
                  <li key={c.conversationId}>
                    <Link
                      ...
                      className={cn(
                        "flex items-center gap-3 px-5 py-3.5 hover:bg-slate-50",
                        active && "bg-indigo-50 hover:bg-indigo-50",
                      )}
                    >
                      <Avatar name={name} colorKey={c.otherUser?.id ?? c.conversationId} />
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-2">
                          ...
                          {c.lastMessage && (
                            <span className={cn("shrink-0 text-xs", unread ? "font-medium text-indigo-600" : "text-slate-400")}>
                              {formatListTime(c.lastMessage.createdAt)}
                            </span>
                          )}
                        </div>
                        <div className="flex items-center justify-between gap-2">
                          <p className={cn("truncate text-xs", unread ? "text-slate-600" : "text-slate-500")}>
